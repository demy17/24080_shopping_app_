package com.onlineshop.app.app.app_fragment
...
import com.onlineshop.app.app.app_adapters.AppCartAdapter
...
class CartFragment : Fragment(),CartInterface {
    ...
    private lateinit var  adapter:AppCartAdapter
    ...
    override fun onCreateView(
        ...
        if(sharedPreferences.getString("dataCart","")?.isNotEmpty() == true){
            ...
            val adapter= AppCartAdapter(requireContext(),dataList)
            ...
            floatingActionButton.setOnClickListener {
                ...
                alertDialog.setPositiveButton("order", DialogInterface.OnClickListener { dialogInterface, i ->
                    run {
                        ...
                        ( requireActivity() as AppHomeActivity).supportActionBar?.subtitle="Total Amount:".plus(sharedPreferences.getString("totalAmount","$0.0"))
    ...
    private fun  showNotification(){
        ...
        val intent= Intent(requireActivity(),AppHomeActivity::class.java)
   val pendingIntent=PendingIntent.getActivity(requireContext(),0,intent,0)
        ...
        notificationBuilder.setContentTitle("Order Notification - Shopping app 24080")
        notificationBuilder.setContentText("Order Successful, Total amount ${ sharedPreferences.getString("totalAmount","0.0$")}")
